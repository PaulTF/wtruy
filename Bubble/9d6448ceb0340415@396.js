  ...
  main.variable(observer("people2")).define("people2", ["stages","d3"], function(stages,d3)
{
  ...
  main.variable(observer("nodes2")).define("nodes2", ["d3","people","groups","radius"], function(d3,people,groups,radius){return(
d3.keys(people).map(function(d) {
  // Initialize count for each group.
  groups[people2[d][0].grp].cnt += 1;
  return {
    id: "node"+d,
    x: groups[people2[d][0].grp].x + Math.random(),
    y: groups[people2[d][0].grp].y + Math.random(),
    r: radius,
    color: groups[people2[d][Math.max(people[d].length -2,1)].grp].color,
  ...
  main.variable(observer("chart")).define("chart", ["d3","width","height","nodes","groups","forceCluster","forceCollide"], function(d3,width,height,nodes,groups,forceCluster,forceCollide)
{
  ...
  main.variable(observer("chart2")).define("chart2", ["d3","width","height","nodes","groups","forceCluster","forceCollide"], function(d3,width,height,nodes,groups,forceCluster,forceCollide)
{
  // Variables.
  var time_so_far = 0;
  // The SVG object.
